...
| D37 | **Ignoring a device keeps it out of the home without forgetting it** | "Adopt everything the network announces" (D29) needs an opposite. An ignored device (`ignored = true` in `devices.toml`) is removed from the registry: not listed, not switchable, nothing recorded, and nothing it says counts as a rejected report. The core keeps what its integration last described and reported, so letting it back in restores it as it is now, straight away, without restarting the integration. It's a core decision rather than each integration's, so every integration gets it; the integration may still talk to the device, which is the honest limit. Asking before adopting new devices at all is the stronger version, still to come. |
...
**Cargo features on the `irori` binary:** `default = ["int-mqtt", "int-demo", "int-esphome", "int-helpers", "ui"]` (D17); opt-in: `assist`, and future `int-zigbee`, `int-matter`, …. `--no-default-features` must still build, start, and serve the API. That's the "robust at its smallest" test.
...
> **Current order (D26):** M0.1 ✅ → M0.2 ✅ → M0.6 ✅ → M1.1 (trimmed) ✅ → M0.8 UI spike ✅ + Devices page ✅ → ESPHome integration ✅ → M0.7 config dir ✅ (rooms, one name and id per device, secrets and encrypted ESPHome devices, ignoring devices, `irori.toml`) → M0.3, M0.4, M0.5.
...
>
> **Not yet:** `extensions/<id>.toml` (nothing needs a setting that isn't secret yet),
> `rules/<id>.json` (M0.3), floors, asking before adopting a newly found device, and helpers
> (values Irori keeps itself, for rules — the Devices page has a placeholder tab).
