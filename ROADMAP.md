...
| D39 | **Irori can ask before adding a device it finds** | `[devices] new` decides what happens to a device nobody has ruled on. `"ask"` holds it back — the same way an ignored one is held (D37), so what its extension says is kept — until a person adds or ignores it; `"add"` puts it straight in. **`"ask"` is the default** (changed from `"add"`): what an extension finds is a proposal, not a decision, and installing an extension should never mean whatever is on the network is now part of the home. The earlier reasoning for `"add"` — that a new install looking empty is a bad first impression — is answered by making the finding visible rather than automatic: each extension's own screen under "+ Add device" lists what it has found, with Add and Ignore on each. It's the answer to D29's "choosing which devices to adopt" and to a neighbour's devices showing up on a shared network. The decision is written to `devices.toml` (`added = true`), so it survives restarts and is visible in the file. Turning asking on marks everything already in the home as added: a setting that emptied the home the moment it was switched on would be read as data loss. |
...
| D46 | **The `zigbee` extension is Irori's first exception to "single static binary with no bundled runtime" (D0, D17), and its first extension needing `host_shell`/broad host trust ahead of both the `app` contribution kind (D22, Phase 3) and auth (D12, M1.5).** It downloads and manages a Node.js runtime and Zigbee2MQTT as a supervised subprocess, with its own embedded MQTT broker (`rumqttd`, loopback-only) so it needs nothing pre-installed — no separate Z2M, no broker, no Z2M frontend ever needed. A generic protocol-action mechanism (`ProtocolContribution.actions`, `ProtocolContext::next_action`/`set_available_actions`) was added alongside it so "+ Add device" can offer a protocol's own dedicated flow — Zigbee's permit-join is its first real use. | No Rust Zigbee radio stack exists to build a native protocol on, unlike `esphome-client`/`rumqttc`, and reimplementing one is out of scope for this project (roughly Home Assistant's own ZHA in size — a multi-year, team-sized effort, mostly because Zigbee2MQTT's device-quirk database is executable JS per device, not portable data). Docker was considered and rejected in favor of raw Node.js/pnpm, accepting the native-module fragility across architectures that Zigbee2MQTT's own maintainers use Docker to avoid — owner's call, revisit if it proves unreliable in practice. Until auth lands, this extension's install endpoint is exactly as exposed as everything else behind `--allow-unauthenticated-lan` — no new hole, but a bigger one than anything reachable through it today, and the manifest says so plainly (`host_shell = true`). |
| D47 | **A failure a person can see must come with a reason they can act on** | `exited exit status: 1` is what the operating system observed, not what went wrong; an extension nearly always printed the real reason — a serial port that isn't there, a setting it couldn't parse — a moment before dying, and inheriting its stderr sent that to Irori's own output where nothing could show it. Every external extension's stderr is piped and read continuously (an unread pipe fills and blocks the child), its recent lines kept, a failure's reason becomes the host's description plus the extension's last words, and the rest is at `GET /api/dev/extensions/<id>/log` and behind a link on its card. The general rule, not a Zigbee fix: the same reasoning made `needs_setup` its own state (D39's neighbour in [extensions.md](docs/specs/extensions.md) §8) rather than a crash loop reporting an exit status. A person running Irori on a Pi in a cupboard has no terminal; if the page can't say why, nobody can. |
| D48 | **A device can be forgotten, and forgetting is one move: off the Devices page, out of the config files, and out of Irori's memory of it** | Issue 33's "remove" is forgetting. Nothing about a device is half-remembered — its `devices.toml` row, its entities in `entities.toml`, the held description a restart would have re-read — so a restart can't resurrect a stale file row either. It's a core decision (as ignore and ask are, D37/D39), so every protocol gets it without writing anything per extension. Forgetting is one-way from Irori's side: the device isn't told, and the honest counterpart is that if it's still out there its extension finds it again — and with asking on (D39) it turns up as a new device waiting to be added, which reads as "forgot it, and it came back" rather than "still secretly in the home". Where ignore keeps the held description so letting back in is instant, forgetting drops it entirely; that's the difference between "keep it out for now" and "stop keeping it at all". Floorplan placement and secrets are deliberately untouched — a secret may be exactly what the device needs when it returns. In the UI it's a Remove behind a confirm on the device page, because forgetting is destructive; the endpoint answers 404 for a device nobody knows. |
| D49 | **Irori's own log is a window in the page, not a file on disk** | Issue 35. D47 gave an extension's own output somewhere to be read; Irori's own words had nowhere. A refused write, a taken port, a `[server]` edit that needs a restart, a plaintext ESPHome connection (D29), a device that vanished off the network — all of it goes to stdout, which belongs to whoever started the process, so on the Pi in the cupboard D47 is about there is no terminal. So the log subscriber's writer now keeps a copy as it formats: the last 400 lines of *this* process, in memory, at `GET /api/dev/system/log`, behind **Show log** on a new Settings section. Deliberately not an archive, and the reasoning is worth keeping: a log on disk is a config decision (where, how long, who may read it, what it costs when a Pi's disk fills) and belongs with `irori logs` and `/metrics` (D18) rather than with a page, while a bounded ring can't fill anything. It's lost on restart, which the section says out loud, and the level is whatever `--log-level` or `[server] log_level` chose at startup (D38) — the page shows what was written, never a filtered or second copy of it, so the honest way to see more is a terminal or `journalctl -u irori`. Extension output is already in it: the core logs every line it inherits with the extension tagged (D47), which is what makes one window enough for both. The endpoint is behind the same unauthenticated `/api/dev/` as the rest (D12), so a log that may name a device, a path or a MAC address is exactly as exposed as the page that already shows those — no new hole, and no new secrecy to keep up. |
...
> D44 — its prose, and the other specs', was reworded to match the same day).
...
>
> A **Logs** section on Settings (➕, issue #35, 2026-09-26) shows the core's own log — the lines
> Irori writes itself, kept in memory as they are formatted, plus each extension's, tagged with the
> extension they came from. It is a window over the running process and not a log archive (D49), so
> it starts again with Irori and keeps only the recent lines.
>
