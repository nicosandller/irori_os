...
> **Current order (D26):** M0.1 ✅ → M0.2 ✅ → M0.6 ✅ → M1.1 (trimmed) ✅ → M0.8 UI spike ✅ + Devices page ✅ → ESPHome integration ✅ → M0.7 config dir ✅ (rooms, names, secrets, encrypted ESPHome devices; `irori.toml` still to come) → M0.3, M0.4, M0.5.
...
>
> Also done: `secrets.toml`, one table per extension, which is how extensions get settings at
...
> `extensions/<id>.toml` (nothing needs a setting that isn't secret yet), `rules/<id>.json` (M0.3),
