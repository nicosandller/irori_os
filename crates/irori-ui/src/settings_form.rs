...
//! blank. Leaving a field blank on submit omits it, so Irori keeps whatever it already had.
...
pub fn SettingsForm(
    ...
    let rows: Vec<(Field, RwSignal<String>)> = fields
        .into_iter()
        .map(|field| {
            let initial = field.default.clone().unwrap_or_default();
            (field, RwSignal::new(initial))
        })
        .collect();
    ...
    view! {
        <form class="settings-form" on:submit=submit>
            {move || trouble.get().map(|why| view! { <p class="banner">{why}</p> })}
            {
                rows.clone()
                    .into_iter()
                    .map(|(field, value)| field_row(field, value))
                    .collect_view()
            }
...
fn field_row(field: Field, value: RwSignal<String>) -> impl IntoView {
